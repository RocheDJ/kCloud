...
def Control_Sequence(step,sdIn,dOut,
                    aIn,
                    dStartTime_StopTime,
                    iHoldTime,iTempTarget,MaxMaxHeatTimeOut):
    # init 
    if (step.step ==0):
        # kill all outputs
        for i in dOut.value:
    ...
    #hold the temp for time
        ...
        dOut.value[2]=Calc_OneInThree(dStartTime_StopTime.value[2])  # agitator on one in 3
